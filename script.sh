...
# Funci車n para manejar errores
handle_error() {
    echo -e "${RED}[!] Error en la l赤nea $1: $2${NC}"
    echo -e "${YELLOW}[*] Soluci車n: Verifica tu conexi車n o reporta el error${NC}"
    exit 1
}

trap 'handle_error $LINENO "$BASH_COMMAND"' ERR

# Verificar conexi車n a internet
check_internet() {
    echo -e "${CYAN}[*] Verificando conexi車n a internet...${NC}"
    if ! ping -c 1 google.com &> /dev/null; then
        echo -e "${RED}[!] Error: Sin conexi車n a internet${NC}"
        echo -e "${YELLOW}[*] Soluci車n: Activa tus datos m車viles o Wi-Fi${NC}"
        exit 1
    fi
    echo -e "${GREEN}[+] Conexi車n a internet confirmada${NC}"
}
...
install_dependencies() {
    ...
    echo -e "\n${CYAN}[*] Instalando dependencias b芍sicas...${NC}"
    ...
    if ! command -v git &> /dev/null; then
        echo -e "${RED}[!] Error: git no se instal車 correctamente${NC}"
        exit 1
    fi
...
main() {
    ...
    # Tu l車gica personalizada aqu赤
    echo -e "\n${CYAN}[*] Ejecutando l車gica personalizada...${NC}"
    ...
    # Ejemplo: Crear archivo de verificaci車n
    echo "Script ejecutado correctamente el $(date)" > success.txt
    echo -e "${GREEN}[+] Archivo de verificaci車n creado: success.txt${NC}"
    
    echo -e "\n${GREEN}=== Ejecuci車n Completa ===${NC}"
    echo -e "${CYAN}Por favor revisa los mensajes arriba para verificar"
    echo -e "que todo se complet車 correctamente."
